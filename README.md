...
> 🌐 **在线体验：[www.133132131.xyz](https://www.133132131.xyz)**
>
...
| **字体自定义** | 标题组/副标题组/页脚组独立设置字体族、字号、颜色，支持8种预设字体+自定义字体，实时预览 |
...
| **9种卡片效果** | 默认/浮雕/凹陷/玻璃拟态/霓虹发光/渐变边框/3D悬浮/简约线条/阴影立体，后台一键切换实时预览 |
...
| **多种排序** | 拖拽排序（当前页）+ 排序号输入（跨页精确排序）+ 上移下移按钮，按分类独立计算序号 |
| **分类管理** | 自定义服务分类名称与排序 |
| **站点设置** | 自定义标题、首页大标题、首页副标题、页脚文字、备案号、主题、字体、背景、Logo、管理员密码 |
...
**方式二：排序号输入（支持跨页）**
1. 在「排序」列输入目标位置数字（从 0 开始）
2. 回车或点击其他地方自动保存
3. 自动跳转到目标所在页
4. 例如输入 `0` 移到该分类最前面

**方式三：上移/下移按钮（支持跨页）**
1. 点击操作列的 **↑** 按钮：和前一个服务交换位置
2. 点击操作列的 **↓** 按钮：和后一个服务交换位置
3. 跨页时自动跳转

> 💡 **提示**：排序号按每个分类独立计算，例如「Synology」分类有6个服务，排序号范围是 0-5。调整某个分类内的顺序不会影响其他分类。
...
1. 进入后台 → **站点设置** → 找到「字体设置」区域
2. 分为三组独立设置：
   - **标题组**：站点标题（导航栏）+ 首页大标题
   - **副标题组**：首页副标题
   - **页脚组**：页脚文字 + 备案号
3. 每组可设置：
   - **字体**：8 种预设字体（思源黑体、微软雅黑、苹方、宋体、楷体、Arial、Georgia、等宽字体），或选择「自定义」输入任意字体名
   - **字号**：数字输入（标题组 12-72px，副标题组 10-48px，页脚组 10-32px）
   - **颜色**：颜色选择器，点击 × 可清除颜色恢复默认
4. 修改后**实时预览**效果
5. 点击 **保存设置** 持久化

> 💡 所有字体设置留空时使用主题默认样式。点击「恢复默认」按钮可一键清除所有字体设置。
